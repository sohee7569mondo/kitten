# -*- coding: utf-8 -*-
import io
H = io.open('H.json', encoding='utf-8').read()
...
HEAD = u'''/* ══════════════════════════════════════════════════════════
   ...
   ★ 앞판을 지우지 않으셔도 됩니다. 이 조각이 나중에 돌아 덮어씁니다.
     다만 헷갈리니 WPCode 에서 앞판(patch160_health)은 꺼두시는 편이
     좋습니다.
...
TAIL = u''';
  ...
  function build(){
    ...
    for(ci=0;ci<NOS.length;ci++){
      ...
      page(head(MARK[ci], e.t2)+h, null, (ci<4?'one':'two'));
    }
