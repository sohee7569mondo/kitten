...
add_action( 'init', function () {
	...
	if ( isset( $_GET['undo'] ) ) {
		...
		/* ★★ wp_slash 로 감싸야 합니다 — 아래 긴 메모를 보세요 */
		wp_update_post( array( 'ID' => $pid, 'post_content' => wp_slash( $old ) ) );
		clean_post_cache( $pid );
		delete_post_meta( $pid, 'stella_done_p%(P)d' );   /* 자물쇠도 풉니다 */
...
FOOT = '''
	...
	if ( get_post_meta( $pid, 'stella_done_p%(P)d', true ) ) {
		...
		echo '<p style="color:#888">정말 다시 넣으셔야 하면 먼저 '
			. '<a style="color:#ffd76a" href="?%(K)s=1&amp;undo=1">되돌리기</a>를 하세요.</p>';
		exit;
	}
	...
	update_post_meta( $pid, 'stella_done_p%(P)d', 1 );
	/* ★★★ 2026-09-16 · 여기서 크게 물렸습니다.
	   wp_update_post 는 글을 저장하며 역빗금을 **한 겹 벗겨냅니다**
	   (안에서 wp_unslash 를 부릅니다). 그래서 넘길 때 wp_slash 로
	   감싸야 본디 글자가 그대로 들어갑니다.

	   안 감쌌더니 쪽 안의 역빗금 다섯 개가 통째로 사라졌고,
	       v.replace(/\+/g, ' ')   →   v.replace(/+/g, ' ')
	   /+/ 는 말이 안 되는 정규식이라 그 script 덩어리가 통째로
	   안 돌았습니다. 검색칸도 여덟 장도 안 나왔습니다.
	   CLAUDE.md 에 「편집기로 저장하면 벗겨진다」고 적어두고선
	   제가 똑같은 짓을 했습니다. */
	wp_update_post( array( 'ID' => $pid, 'post_content' => wp_slash( $new ) ) );

	/* 정말 그대로 들어갔는지 다시 읽어 역빗금을 셉니다 */
	clean_post_cache( $pid );
	$chk  = get_post( $pid );
	$bsIn = substr_count( $page->post_content, chr( 92 ) );
	$bsOut = substr_count( $chk->post_content, chr( 92 ) );
	clean_post_cache( $pid );
	if ( function_exists( 'wp_cache_flush' ) ) { wp_cache_flush(); }

	echo '<h2 style="color:#7bff9b">넣었습니다.</h2>';
	printf( '<p>역빗금 넣기 전 <b>%%d개</b> → 넣은 뒤 <b style="color:%%s">%%d개</b>%%s</p>',
		$bsIn, ( $bsIn === $bsOut ) ? '#7bff9b' : '#ff7b7b', $bsOut,
		( $bsIn === $bsOut ) ? ' (그대로입니다)' : ' ★ 벗겨졌습니다 — 저에게 알려주세요' );
