 ...
 * 4.6 메일로 받는 로그인 링크
 *
 *   가입할 때 비밀번호를 만들지 않았으니, 돌아오실 때도 묻지 않습니다.
 *   이메일만 넣으시면 30분짜리 링크를 보내드리고, 그 링크를 누르면 들어오십니다.
 *
 *   · 열쇠는 해시로만 저장합니다 (메일이 새도 데이터베이스에서 되찾을 수 없게)
 *   · 한 번 쓰면 지워집니다
 *   · 가입 안 된 주소를 넣어도 똑같이 대답합니다 — 누가 회원인지 알려주지 않습니다
 * --------------------------------------------------------------- */

/**
 * 이 계정이 메일 링크로 들어와도 되는 계정인가.
 *
 * 손님(구독자)은 됩니다. 관리자·편집자처럼 사이트를 고칠 수 있는 계정은
 * 안 됩니다. 메일 한 통이 곧 열쇠가 되는 방식이라, 메일함이 털리면
 * 사이트까지 통째로 넘어갑니다. 그런 계정은 워드프레스 본래의
 * 로그인 화면(/wp-login.php)을 쓰셔야 합니다.
 */
...
/** 로그인 링크를 만들어 메일로 보냅니다 */
function stella_login_send( $uid ) {
	$user = get_userdata( $uid );
	if ( ! $user ) {
		return false;
	}
	if ( ! stella_login_allowed( $uid ) ) {
		return false;
	}
	$token = wp_generate_password( 32, false, false );
	update_user_meta( $uid, 'stella_login_hash', wp_hash_password( $token ) );
	update_user_meta( $uid, 'stella_login_till', time() + STELLA_LOGIN_MINUTES * MINUTE_IN_SECONDS );

	$link = add_query_arg(
		array( 'stella_login' => $token, 'u' => $uid ),
		home_url( '/' )
	);
	$name = (string) get_user_meta( $uid, 'stella_name', true );
	$name = $name !== '' ? $name : $user->display_name;
	...
	$lines   = array();
	$lines[] = $name . '님, 아래 링크를 누르시면 바로 들어오실 수 있어요.';
	$lines[] = '';
	$lines[] = $link;
	$lines[] = '';
	$lines[] = '이 링크는 ' . STELLA_LOGIN_MINUTES . '분 동안만 살아 있고, 한 번 쓰면 사라집니다.';
	$lines[] = '비밀번호는 따로 없어도 됩니다.';
	$lines[] = '';
	$lines[] = '혹시 직접 요청하신 것이 아니라면 이 메일은 그냥 지우셔도 됩니다.';
	$lines[] = '누군가 주소를 잘못 적었을 뿐, 계정에는 아무 일도 일어나지 않습니다.';
	$lines[] = '';
	$lines[] = '— 명성의 신 드림';
...
/**
 * 이 열쇠가 맞는지 봅니다. 맞든 틀리든 열쇠는 지웁니다 — 한 번 쓰면 끝입니다.
 *
 * @return bool
 */
function stella_login_check( $uid, $token ) {
	if ( $uid < 1 ) {
		return false;
	}
	if ( $token === '' ) {
	...
	$hash = (string) get_user_meta( $uid, 'stella_login_hash', true );
	$till = (int) get_user_meta( $uid, 'stella_login_till', true );

	delete_user_meta( $uid, 'stella_login_hash' );
	delete_user_meta( $uid, 'stella_login_till' );
	...
	return (bool) wp_check_password( $token, $hash, $uid );
}

/** 링크를 누르고 들어오셨을 때 */
add_action( 'init', function () {
	if ( ! isset( $_GET['stella_login'] ) ) {
		return;
	}
	$token = sanitize_text_field( wp_unslash( $_GET['stella_login'] ) );
	$uid   = isset( $_GET['u'] ) ? (int) $_GET['u'] : 0;

	if ( ! stella_login_check( $uid, $token ) ) {
		wp_safe_redirect( home_url( '/login/?again=1' ) );
		exit;
	}

	/* 보낼 때 막았지만, 들어올 때 한 번 더 봅니다.
	   그 사이에 그 계정이 관리자가 되었을 수도 있으니까요. */
	if ( ! stella_login_allowed( $uid ) ) {
...
add_action( 'rest_api_init', function () {
	...
	register_rest_route( 'stella/v1', '/login', array(
		'methods'             => 'POST',
		'permission_callback' => '__return_true',
 ...
 * 로그인 링크 보내주세요.
 *
 * 가입된 주소든 아니든 대답은 똑같습니다.
 * 여기서 '그런 회원 없습니다' 라고 하면, 아무나 이메일을 넣어보며
 * 누가 우리 회원인지 알아낼 수 있게 됩니다.
 */
function stella_rest_login( WP_REST_Request $req ) {
	$ip  = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : '0';
	$key = 'stella_lg_' . md5( $ip );
	...
	$said = '메일을 보내드렸어요. 받은 편지함을 확인해주세요.';
	...
	if ( $user ) {
		/* 같은 사람에게 자꾸 보내지는 않습니다 */
		$one = 'stella_lg1_' . $user->ID;
		if ( ! get_transient( $one ) ) {
			set_transient( $one, 1, 2 * MINUTE_IN_SECONDS );
			stella_login_send( $user->ID );
		}
	}

	return rest_ensure_response( array( 'ok' => true, 'message' => $said ) );
}
...
function stella_rest_me( WP_REST_Request $req ) {
	...
	return rest_ensure_response( array(
		...
		/* 머리말이 '로그인'을 '로그아웃'으로 바꿀 때 쓰는 주소입니다.
		   머리말은 원래 wp_head 가 심어준 window.STELLA_USER 를 보는데,
		   페이지가 캐시되어 있으면 그게 아예 안 실려 옵니다.
		   그때는 머리말이 이 /me 를 직접 물어봅니다 — 이 길은 캐시를 타지 않습니다. */
		'logout'   => wp_logout_url( home_url( '/' ) ),
		'pay'      => array(
			'ready'    => stella_pay_ready(),
			'currency' => strtolower( (string) STELLA_PAY_CURRENCY ),
		),
		/* 마이페이지가 '정보 고치기'를 저장할 때 나라를 잃지 않도록 함께 내려줍니다.
		   그 화면에는 나라를 고르는 칸이 없어서, 없으면 한국으로 덮어써버립니다.
		   밴쿠버에서 태어난 분이 한국 기준으로 계산되던 문제가 여기서 났습니다. */
		'profile'  => stella_profile_of( $uid ),
...
function stella_rest_signup( WP_REST_Request $req ) {
	...
	if ( $existing ) {
		if ( ! is_user_logged_in() ) {
			/* 돌아오신 분입니다. 비밀번호가 없으니 메일로 들어올 길을 열어드립니다. */
			$one = 'stella_lg1_' . $existing->ID;
			if ( ! get_transient( $one ) ) {
				set_transient( $one, 1, 2 * MINUTE_IN_SECONDS );
				stella_login_send( $existing->ID );
			}
			return new WP_Error(
				'stella_exists',
				'이미 쓰고 계신 이메일이에요. 들어오실 수 있는 링크를 방금 메일로 보내드렸습니다. ' .
				'메일을 열어 링크를 누르시면 그동안 받으신 풀이와 구슬이 그대로 있어요.',
				array( 'status' => 409 )
			);
		}
		$uid = $existing->ID;
...
add_action( 'wp_head', function () {
	...
	$data = array(
		...
		'years'   => (string) get_user_meta( $uid, 'stella_years', true ),
		'want'    => (string) get_user_meta( $uid, 'stella_want', true ),
		/* 머리말의 '로그아웃'이 쓸 주소입니다. 워드프레스가 만든 것이라
		   한 번 쓰는 열쇠가 붙어 있어, 남이 몰래 로그아웃시킬 수 없습니다. */
		'logout'  => wp_logout_url( home_url( '/' ) ),
	);
