...
OLD = ("    var seed = (P.year||0)*10007 + (P.month||0)*331 + (P.day||0)*97\n"
       "             + (P.hour||0)*13 + (P.minute||0)*7\n"
       "             + now.getFullYear()*1103 + (now.getMonth()+1)*37 + now.getDate()*3 + 11;")

NEW = ("    /* 2026-08-31 · 주제를 섞는 씨앗에 더합니다.\n"
...
add_action( 'admin_init', function () use ( $STELLA_D_RE, $STELLA_D_NEW ) {
	...
	$say( array_merge( $log, array( '',
		...
		'2) 호스팅 → 성능 → 캐시 비우기',
		'3) 시크릿 창에서 인연을 잇는 신 → 결혼운으로 한 번, 연애운으로 한 번',
...
php = PHP.replace('__RE__', nowdoc('RE', '~' + re.escape(OLD) + '~s'))\
         .replace('__NEW__', nowdoc('NEWJS', NEW))
open('/home/user/kitten/wordpress/php/patch137_deck.php', 'w', encoding='utf-8').write(php)
open('anchor137.txt', 'w', encoding='utf-8', newline='').write(OLD)
open('new137.txt', 'w', encoding='utf-8', newline='').write(NEW)
print('만들었습니다. 늘어날 바이트 :', len(NEW.encode()) - len(OLD.encode()))
