...
(function () {
  ...
  /* 카드에 실을 두 사람 이름.
     한글·영문·숫자만 남기고 여섯 자까지 자릅니다.
     이름을 서버로 넘기는 방법은 주소의 뒷칸입니다 —
       /um-card/12-lover-6-3-3/소희/정호
     주소 표준 그대로라 서버는 풀어낼 것이 없습니다. */
  function safeName(s) {
    var t = String(s || '').replace(/[^가-힣a-zA-Z0-9]/g, '');
    return t.slice(0, 6);
  }
  function cardNames() {
    if (!st.result) { return null; }
    var a = safeName(st.me ? st.me.name : '');
    var b = safeName(st.you ? st.you.name : '');
    if (!a) { return null; }
    if (!b) { return null; }
    return [a, b];
  }
  /* 파일 이름에 쓸 이름표 — 영문·숫자·밑줄만 남깁니다.
     서버도 똑같은 규칙으로 만들어서 두 쪽이 늘 같은 파일을 봅니다. */
  function nameKey(s) {
    return encodeURIComponent(s).replace(/[^A-Za-z0-9]/g, '_');
  }
  function cardName() {
    if (!st.result) { return ''; }
    var r = st.result;
    var n = r.total + '-' + st.rel + '-' + r.blocks.saju.score + '-' +
      r.blocks.zodiac.score + '-' + r.blocks.animal.score;
    var nm = cardNames();
    if (nm) { n += '-' + nameKey(nm[0]) + '-' + nameKey(nm[1]); }
    return n;
  }
  /* 주소 뒤에 붙는 이름 두 칸 */
  function cardTail() {
    var nm = cardNames();
    if (!nm) { return ''; }
    return '/' + encodeURIComponent(nm[0]) + '/' + encodeURIComponent(nm[1]);
  }
  /* 우리 서버가 그 자리에서 그려 주는 주소 */
  function cardUrl() {
    var n = cardName();
    return n ? (location.origin + '/um-card/' + n + cardTail()) : '';
  }
  ...
  function cardWarm() {
    ...
    try {
      ...
      /* 물음표가 붙은 주소가 막히는 일이 있어 경로식으로 부릅니다 */
      x.open('GET', location.origin + '/um-kimg/' + n + cardTail(), true);
      x.onload = function () {
        try {
  ...
  function kakao(url, text) {
    /* 카톡에도 결과 주소를 그대로 보냅니다.
       한동안 짧은 주소로 줄여 보냈던 것은, 긴 주소를 누르면 아무
       반응이 없어서 카카오가 길이를 싫어하는 줄 알았기 때문입니다.
       진짜 원인은 카카오 콘솔의 제품 링크 관리 → 웹 도메인이 비어
       있던 것이었습니다. 그것을 채우니 긴 주소도 잘 열립니다.
       그래서 받는 사람이 누르면 첫 화면이 아니라 결과가 보입니다. */
  ...
  function celebSheet() {
    ...
    /* 남자 · 여자만 가려 보기 — 335명을 훑는 것보다 빠릅니다 */
    var sex = '';
    var chips = el('<div style="display:flex;gap:7px;margin-top:10px"></div>');
    function chip(v, t) {
      var b = el('<button style="border:0;border-radius:999px;padding:8px 16px;' +
        'font-size:12.5px;font-weight:600;font-family:inherit;background:#fff;' +
        'color:#8C82A6">' + t + '</button>');
      if (!v) { b.style.background = '#6B5BA8'; b.style.color = '#fff'; }
      b.addEventListener('click', function () {
        sex = v;
        var bb = chips.querySelectorAll('button'), j;
        for (j = 0; j < bb.length; j++) {
          bb[j].style.background = '#fff';
          bb[j].style.color = '#8C82A6';
        }
        b.style.background = '#6B5BA8';
        b.style.color = '#fff';
        draw(inp.value.replace(/\s+/g, ''));
      });
      chips.appendChild(b);
    }
    chip('', '전체');
    ...
    var body = el('<div style="flex:1 1 auto;overflow-y:auto;padding:4px 18px 30px;' +
      'display:grid;grid-template-columns:repeat(auto-fill,minmax(148px,1fr));' +
      'gap:7px;align-content:start"></div>');
    ...
    function draw(q) {
      ...
      for (i = 0; i < all.length; i++) {
        ...
        if (sex) { if (p.s !== sex) { continue; } }
        if (n >= 220) { break; }
        if (p.g !== cur) {
          cur = p.g;
          out.push('<div style="grid-column:1/-1;font-size:12px;font-weight:700;' +
            'color:#A79BC4;margin:16px 0 3px;letter-spacing:.02em">' + esc(p.g) + '</div>');
        }
        out.push('<button class="um-cb" data-i="' + i + '" style="text-align:left;min-width:0;' +
          'background:#fff;border:0;border-radius:15px;padding:12px 13px;font-family:inherit">' +
          '<div style="font-size:14.5px;font-weight:600;color:#3F3A52;white-space:nowrap;' +
          'overflow:hidden;text-overflow:ellipsis">' + esc(p.n) + '</div>' +
          '<div style="font-size:11px;color:#B3A9C9;margin-top:3px">' +
          p.y + '.' + pad2n(p.m) + '.' + pad2n(p.d) + '</div></button>');
        n++;
      }
      if (!n) {
        out.push('<div style="grid-column:1/-1;text-align:center;color:#A79BC4;' +
  ...
  function render() {
    ...
    if (st.view === 'home') {
      ...
      '<div class="pad" style="padding-top:26px;padding-bottom:18px;position:relative">' +
        peek('히어로큰', 'bunny_pink', 134, 'right:6px;top:8px') +
      ...
      '<div class="pad" style="margin-top:100px;position:relative">' +
        pop('공짜카드', 118, 'right:14px;top:-72px') +
        '<div class="card front">' +
      ...
      h.push('<div class="pad" style="padding-top:26px"><div class="card">',
        ...
        '</div></div>',
        (st.joined ? ('<div class="pad" style="margin-top:24px"><div class="lbl">' +
          esc(st.me.name || '상대') + '님에게 보내기</div>' +
          '<div class="card" style="padding:20px">' +
          '<div style="font-size:13.5px;line-height:1.85;color:#57506E">' +
          esc(st.me.name || '상대') + '님은 아직 이 점수를 못 봤어요. ' +
          '보내주면 그 사람 화면에도 같은 결과가 그대로 뜹니다.</div>' +
          '<div style="height:14px"></div>' +
          '<button class="btnP" id="backto">결과 보내기</button>' +
          '</div></div>') : ''),
  ...
  function wire() {
    ...
    bind('#backto', function () {
      var url = resultUrl();
      var nm = (st.you ? (st.you.name || '') : '') || '상대';
      var txt = nm + '님이 답했어요 · ' + st.result.total + '점';
      if (kakaoKey()) { kakao(url, txt); return; }
      if (nativeShare(url, txt)) { return; }
      copy(url); toast('링크를 복사했어요. 그 사람에게 보내주세요');
    });
    bind('#again', function () {
      st.joined = false;
  ...
  function boot() {
    ...
    if (r) {
      ...
      if (d1) { st.rel = d1.rel; st.me = d1.a; st.joined = true; st.view = 'joined'; draft = { sex: 'M', known: true, ampm: '오전' }; }
    }
    render();
