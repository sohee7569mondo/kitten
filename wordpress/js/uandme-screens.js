...
(function () {
  ...
  function drawResult(P, cb) {
    ...
    drawPic(g, P[slotA2], 166, 448, 268);
    drawPic(g, P['__lucky'], 914, 448, 268);
  ...
  function drawInvite(P, cb) {
    ...
    drawPic(g, P[slotB2], 166, 466, 268);
    drawPic(g, P['__lucky'], 914, 466, 268);
  ...
  /* 캔버스에 그림을 넣습니다 — 빈 곳은 빼고 상자에 꽉 채웁니다 */
  function drawPic(g, im, cx, cy, box) {
    if (!im) { return; }
    var t = trimBox(im);
    if (!t.w) { return; }
    var k = Math.min(box / t.w, box / t.h);
  ...
  function render() {
    ...
    if (st.view === 'home') {
      ...
      '<div class="pad" style="margin-top:30px"><button class="btnP" id="go-mine">' + T('시작버튼') + '</button>',
        '<div style="height:10px"></div><button class="btnG" id="celeb0" style="background:#F3ECFF;color:#6B5BA8">연예인이랑 볼래요</button></div>',
