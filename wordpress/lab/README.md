...
★ 고친 곳 셋
    ...
    · introSub 의 「당신의 선택이」 -> 「도쿄 3박 4일, 여덟 번만 고르면
      내 MBTI가 나와요」. 「당신」을 쓰지 않기로 한 규칙.
    · 결과 머리말 「나의 도쿄 여행 유형」 -> 「도쿄에서 알아본 내 MBTI」
    · 공유 글 「내 도쿄 여행 유형은 …」 -> 「도쿄 여행으로 알아본 내 MBTI는 …」

  ★ 소희 님 : 「mbti를 일본여행을 이용해서 알아본거야」
    — 여행 유형을 재는 검사가 아니라, 여행이라는 상황을 빌려
      MBTI 를 재는 검사입니다. 말이 그렇게 읽혀야 합니다.

## 2026-09-09 · 목록도 스니펫으로

지금 `/lab/` 에 보이는 것은 본체(804)가 그리는 옛 목록입니다.
`labindex.html` 은 만들어만 두고 사이트에 안 올라가 있었습니다.

    snippets/snip_lab_index.WPCODE.txt   -> /lab/
...
목록을 먼저 넣으면 새 카드를 눌러도 빈 쪽으로 갑니다.

### 목록에 열 장을 더했습니다

랩에 이미 있던 검사들이 새 목록에 없어서, 목록을 바꾸면 갈 데가
없어집니다. 「글로 보는 검사」라는 묶음으로 아래에 넣었습니다.

    love · agegap · drama · katok · stress · color · mouse
    ohaeng · sipseong · mbti
...
### 확인한 것
    ...
    · /lab 과 /lab/ 에서만 열림
    · /lab/mbti/ · /lab/tokyo/ · /laboratory/ · / 에서는 아무것도 안 내놓음
      (본체가 맡는 검사 주소를 안 가로챕니다)
    · 카드 열일곱 장 모두 주소가 붙어 있음
    · 390px 가로 넘침 없음
    · 이 스니펫을 끄면 옛 목록으로 그대로 돌아감
